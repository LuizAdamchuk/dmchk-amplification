/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { WorkspaceWhereUniqueInput } from "../../workspace/base/WorkspaceWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class WorkspaceUpdateManyWithoutOrganizationsWorkspacesInput {
  @Field(() => [WorkspaceWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [WorkspaceWhereUniqueInput],
  })
  connect?: Array<WorkspaceWhereUniqueInput>;

  @Field(() => [WorkspaceWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [WorkspaceWhereUniqueInput],
  })
  disconnect?: Array<WorkspaceWhereUniqueInput>;

  @Field(() => [WorkspaceWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [WorkspaceWhereUniqueInput],
  })
  set?: Array<WorkspaceWhereUniqueInput>;
}

export { WorkspaceUpdateManyWithoutOrganizationsWorkspacesInput as WorkspaceUpdateManyWithoutOrganizationsWorkspacesInput };
